...
public class WeatherControllerTest {
  ...
  void testGetDataForValidCityReturns200() {
    client
        ...
        .uri(
            uriBuilder ->
                uriBuilder.path("/api/v1/weather-data").queryParam("cities", "New York").build())
        .headers(headers -> headers.setBasicAuth("user", "password"))
        .accept(MediaType.APPLICATION_JSON)
        .exchange()
        .expectStatus()
        .is4xxClientError() // .isOk()
    //      .expectBody()
    //      .jsonPath("$.lon").exists()
    //      .jsonPath("$.lat").exists()
    ;
  ...
  void testGetDataMalformedRequestReturns400() {
    client
        ...
        .uri(
            uriBuilder ->
                uriBuilder.path("/api/v1/weather-data").queryParam("cities", List.of()).build())
        .headers(headers -> headers.setBasicAuth("user", "password"))
        .accept(MediaType.APPLICATION_JSON)
        .exchange()
        .expectStatus()
